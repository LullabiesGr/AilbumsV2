...
const API_URL = 'http://localhost:8000'; // Update this to your actual backend URL
...
export const getUserAlbums = async (userId: string): Promise<SavedAlbum[]> => {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(errorText || 'Failed to fetch albums');
    }
...
export const saveAlbumToBackend = async (albumData: SaveAlbumData): Promise<void> => {
  const formData = new FormData();
  ...
  formData.append('created_date', albumData.created_date);
  formData.append('approved_paths', JSON.stringify(albumData.approved_paths));
  formData.append('ratings', JSON.stringify(albumData.ratings));
  formData.append('tags', JSON.stringify(albumData.tags));
  formData.append('highlights', JSON.stringify(albumData.highlights));
  formData.append('color_labels', JSON.stringify(albumData.color_labels));
  formData.append('edits', JSON.stringify(albumData.edits));
  formData.append('thumbnails', JSON.stringify(albumData.thumbnails));
...
export const loadAlbumFromBackend = async (albumId: string, userId: string): Promise<SavedAlbum> => {
  try {
    const response = await fetch(`${API_URL}/album/${albumId}?user_id=${userId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(errorText || 'Failed to load album');
    }
