  ...
  const renderCardContent = () => (
    <>
      ...
      <div className="p-3">
        ...
        {/* Event Highlights - First Priority with Gold Gradient */}
        {photo.blip_highlights && photo.blip_highlights.length > 0 && (
        ...
        {/* Secondary Tags - Neutral styling below highlights */}
        {photo.tags && photo.tags.filter(tag => tag !== 'raw').length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {photo.tags.filter(tag => tag !== 'raw').map((tag, index) => {
              const icon = getTagIcon(tag);
              return (
                <span 
                  key={index} 
                  className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-xs rounded flex items-center gap-1"
                >
                  {icon}
        ...
        {/* RAW tag separately if exists */}
        {photo.tags?.includes('raw') && (
          <div className="mt-1">
            <span className="px-2 py-1 bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200 text-xs rounded font-medium">
              RAW
            </span>
          </div>
        )}
        
        {/* Flags and Issues - Red styling */}
        {photo.blip_flags && photo.blip_flags.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {photo.blip_flags.map((flag, index) => (
              <span 
                key={`flag-${index}`}
                className="px-2 py-1 bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200 text-xs rounded font-medium flex items-center gap-1"
        ...
        {/* AI Categories and Status */}
        {(photo.ai_categories || photo.approved !== undefined || photo.score_type === 'ai') && (
          <div className="mt-2 flex flex-wrap gap-1">
            ...
            {photo.approved && (
              <span className="px-2 py-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 text-xs rounded font-medium flex items-center gap-1">
            ...
            {photo.score_type === 'ai' && (
              <span className="px-2 py-1 bg-purple-100 dark:bg-purple-900 text-purple-600 dark:text-purple-400 text-xs rounded font-medium">
                Deep AI
              </span>
            )}
          </div>
    ...
    >
      ...
      <div className="flex-1 p-3 flex flex-col justify-between">
        <div>
          ...
          {photo.blip_highlights && photo.blip_highlights.length > 0 && (
            <div className="mt-1 flex flex-wrap gap-1">
              {photo.blip_highlights.slice(0, 2).map((highlight, index) => (
                <span 
                  key={`highlight-${index}`}
                  className="px-1.5 py-0.5 bg-gradient-to-r from-yellow-400 to-orange-400 text-white text-xs rounded-full font-medium flex items-center gap-1"
                >
                  {getHighlightIcon(highlight)}
                  <span>{cleanTagLabel(highlight)}</span>
                </span>
              ))}
            </div>
          )}
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            {getFaceSummaryBadges()}
            {photo.blip_flags && photo.blip_flags.slice(0, 1).map((flag, index) => (
              <span 
                key={`flag-${index}`}
