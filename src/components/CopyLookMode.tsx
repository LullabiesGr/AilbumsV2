...
// Επιτρέπουμε info άλμπουμ στο Photo
type PhotoWithAlbum = Photo & {
  album?: string;      // π.χ. "lullabiesgr@gmail.com/reyb"
  album_path?: string; // ίδιο νόημα αν το έχεις έτσι
};
...
/** Fetch -> Blob -> File (μόνο αν είναι image/*) */
...
/** Επιστρέφει ΣΙΓΟΥΡΑ σωστό File από Photo:
...
async function fileFromPhoto(p: PhotoWithAlbum): Promise<File> {
  ...
  if (p.file && p.file.size > 0) {
    ...
    const buf = await p.file.arrayBuffer(); // fresh body για να μη θεωρηθεί consumed
    return new File([buf], cleanName(p.filename), { type });
  }

  // 2) δοκίμασε από url (blob:/data:/http)
  if (p.url) {
    try {
      return await urlToFile(p.url, p.filename);
    } catch {
      /* προχωράμε σε fallback */
    }
  }
...
const CopyLookMode: React.FC<CopyLookModeProps> = ({ onBack }) => {
  ...
  const handleApplyCopyLook = async () => {
    ...
    try {
      ...
      // 2) για κάθε target: source/apply_on + POST
